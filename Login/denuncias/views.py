...
from .forms import DefensoriasForm, EstadoForm, TipologiaForm, TipoForm, ActuacionFormModel, DenunciaForm
...
def iniciar_denuncia(request):
	if request.method == 'POST':
		...
		if form_denu.is_valid():
			form_denu.save(commit = False)
			form_denu.completa = False
			form_denu.save()
